...
function http_(proto) {
  ...
  function request(runner, step, data, expected) {
    ...
    runner.debug('\n' + runner.id + ' out --> ' + step.action + ':', this.proto, '\n', options, '\n', data.queryData);
...
function onInputEvent(emitter, event, data) {  // 
  ...
  runner.debug('\n' + runner.id + ' <-- in: ' + emitter.name, event, JSON.stringify (data));
  ...
  if (emitter.expected instanceof Array) {
    for (var i=-1; ++i < emitter.expected.length;) {
      ...
      if (runner_.checkData([event, data], emitter.expected[i], runner.parameters)) {
        ...
        runner.debug('??? checkData failed', emitter.expected[i]);
      }
    }

    if (runner.haveToWait[0] && (emitter.expected.length < 1)) {
      clearTimeout(runner.haveToWait[0]);
      delete runner.haveToWait[0];     
