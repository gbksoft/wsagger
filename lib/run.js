...
if (inChain) {
  ...
  try {
    ...
    // data = json.parse(fs.readFileSync(dataFile, {encoding: 'utf8'}), null, true);  
    data = jsonlint.parse(fs.readFileSync(dataFile, {encoding: 'utf8'}), null, true);  
  ...
  scenario = runner_.prepareScenario(data, server, user);
...
function initNodeLibraries (runner, debugMode) {
  ...
  if ('doer.process'   in runner.parameters) runner.parameters['doer.process']   = new process_();
...
function call_() {
  ...
  function callWsagger(runner, step, data, expected, fromFile) {
    ...
    try {
      ...
      if (fromFile) {
         runner.debug('\nfrom file <-- :', wsagger_);    
         var wsaggerScript_ = fs.readFileSync(wsagger_, { encoding: "utf8"});
    ...
    try {
      ...
      runner.debug('\nout --> ' + step.action + ':', wsaggerScript, data);    
...
function exec_() {
  ...
  function exec_(runner, step, data, expected, async) {   
    try {
      ...
      if (async) {
         runner.numWorkers++;
         var child = exec.node_.fork(command);

         // , (err, stdout, stderr) => { processStdio('execAsync', runner, step, stdout, stderr, expected, err); }

      } else {
         processStdio('execSync', runner, step, exec.node_.execSync(command, {encoding: 'utf8'}), undefined, expected);
...
function process_() {
  var proc          = this;
  proc.emitToItself = emitToItself;
  proc.sendToParent = sendToParent;
  proc.expect       = expect;
  proc.name         = 'process';

  process.on('warning', (warning) => {
    onInputEvent(proc, 'warning', JSON.parse(warning.message));
  });
  
  process.on('message', (data) => {
    onInputEvent(proc, 'message', data);
  });

  return proc;
  
  function emitToItself(runner, step, data, expected) {
    proc.runner   = runner;    
    runner.debug('\nout --> ' + step.action + ':', data);
    proc.expected = expected;    
    process.emitWarning(JSON.stringify(data));
  }

  function sendToParent(runner, step, data, expected) {
    proc.runner   = runner;    
    runner.debug('\nout --> ' + step.action + ':', data);
    proc.expected = expected;    
    process.send(data);
...
function onInputEvent(emitter, event, data) {  // 
  var runner = emitter.runner;
  runner.debug('\n<-- in: ' + emitter.name, event, JSON.stringify (data));
    
  if (emitter.expected instanceof Array) {
    for (var i=-1; ++i < emitter.expected.length;) {
      if (runner_.checkData([event, data], emitter.expected[i], runner.parameters)) {
        runner.debug('! expected !');
        emitter.expected.splice(i, 1);
        break;
    ...
    if (runner.haveToWait && (emitter.expected.length < 1)) {
    ...
    // runner.debug('onInputEvent: no expectations', emitter.expected);
