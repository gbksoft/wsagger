...
if (inChain) {
  ...
  var data     = loadObject(process.argv[2]);
  var variants = loadObject(process.env.WSAGGER_SCRIPT_PATH + '/_variants.json');
 
  var server   = process.argv[3];
  var user     = process.argv[4];

  scenario     = runner_.prepareScenario(data, variants, server, user);
}
  ...
  if (runner.inChain) {
    ...
    if (runner.dataOut != undefined) {
      if (runner.dataOut && ((typeof runner.dataOut) == 'object')) {
        for (var k in runner.dataOut) data[k] = runner_.setParameters(runner.dataOut[k], runner.parameters); 
...
function http_(proto) {
  ...
  function request(runner, step, data, expected) {
    data = object_((data instanceof Array) ? data[0] : data);
    ...
    if ((!data.queryData) && data.queryData_) {
      if (!d.data.headers['Content-Type']) { d.data.headers['Content-Type'] = ''; }

      if (typeof data.queryData_ !== 'object') {
        data.queryData = data.queryData_;
      
      } else if (d.data.headers['Content-Type'].indexOf('application/x-www-form-urlencoded') >= 0) {
        data.queryData = querystring.stringify(data.queryData_);
      
      } else {
        data.queryData = JSON.stringify(data.queryData_);
        if (d.data.headers['Content-Type'].indexOf('application/json') < 0) { d.data.headers['Content-Type'] = 'application/json'; }
    ...
    var req = this.node_.request(options, (res) => {
      ...
      res.on('end',   ()      => {
        ...
        if (res.headers['content-type'] && (res.headers['content-type'].indexOf('application/json') >= 0)) {  
           ...
           runner._callback(runner, step, require('querystring').parse(responseData), expected);       

        } else {
           runner._callback(runner, step, { headers: res.headers, body: responseData }, expected);         
