...
function _run(runner) {
  ...
  while (runner.step = runner.data.shift()) {
    ...
    if (runner.step.action) {
      var data = setParameters(runner.step.data, runner.parameters);  
      var doer = doer_(runner.step);
      
      if (doer && runner.parameters['doer.' + doer[0]]) {
        if (doer[1]) {
          var action = runner.parameters['doer.' + doer[0]][doer[1]];
          action.apply(runner.parameters['doer.' + doer[0]], [runner, runner.step, data, runner.step.expected] );
    
        } else {
          runner.parameters['doer.' + doer[0]].apply([runner, runner.step, data, runner.step.expected]);
...
function _callback(runner, step, data, expected, err) {
  ...
  if (runner.isError) {
    ...
    runner.error('\n' + runner.id + ' /_callback(...err):', ((err_.length > 1000) ? err_.substr(0, 1000) + '...............' : err_));
    runner._finish(runner, false);
