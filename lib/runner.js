...
function prepareScenario(scenario, server, user, addFormData) {
  log('prepareScenario...');
  ...
  if (server) {
    var REST_  = object_(scenario.REST_);      
    ...
    var server_  = object_(scenario.server_);
  ...
  if (user) { 
    var user_ = object_(scenario.user_);
...
function _run(runner) {
  ...
  if (runner.haveToWait[0]) {
    runner.error('\n' + runner.id + ' /runScenario: runner.haveToWait', runner); 
    return;
  }
  ...
  while (step = runner.data.shift()) {
    ...
    runner.debug('\n' + runner.id + ' STEP ', ++runner.stepNum, '\n\n', step);

    // showP(runner);

    var expected = setParameters(step.expected, runner.parameters);

    if (step.wait)      runner.haveToWait[0]              = _stopWaiting(runner, step, expected, step.wait.delay,      0);
    if (step.waitAsync) runner.haveToWait[runner.stepNum] = _stopWaiting(runner, step, expected, step.waitAsync.delay, runner.stepNum);
...
function _stopWaiting(runner, step, expected, delay, stepNum) {
  ...
  var async = stepNum ? 'Async' : '';

  runner.debug('\n' + runner.id + ' setting wait' + async + ' timeout ' + stepNum);
  return setTimeout(
    () => { 
      runner.debug('\n' + runner.id + ' !!! wait' + async + ' timeout ' + stepNum + ' !!!!', expected);
...
function _finish(runner, success) {
  if (success) {
    if (--runner.numWorkers) {
      runner.debug('\n' + runner.id + ' !!! worker finished successfully !!!\n');       
      return;
    }
