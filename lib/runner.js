...
function prepareScenarioCall(data, scenarioNum, parameters) {
  data = object_(data);

  var scenario = {parameters: {}, out: data.out};
  ...
  if (s_[scenarioNum] && (typeof s_[scenarioNum] === 'object') && s_[scenarioNum].flow) {
    ...
    for (var key of array_(data.in)) scenario.parameters[key] = parameters[key];
    return scenario;
  }    
...
function _finishCall(runner) {
  var data = {};

  runner.parameters.doers = runner.doers;

  for (var key of array_(runner.out)) data[key] = runner.parameters[key];
  return data;
}  
...
function initScenario (scenario, initLibraries, debugMode) { 
  ...
  runner._finishCall   = _finishCall;  
...
function _run(runner) {
  ...
  while (step = runner.flow.shift()) {
    runner.debug ("\nSTEP ", ++runner.stepNum, 777777777, runner.flow, 88888888888, '\n\n', step);
...
function doStep(runner, step) {
  ...
  if (step.wait) {
    ...
    // runner.haveToWait = setTimeout(() => { stopWaiting(runner, step, expected); }, step.wait.delay);    
  }
  ...
  if (step.action) {
    ...
    if (doer && runner.doers[doer[0]]) {
      if (doer[1]) {
        ...
        action.apply(runner.doers[doer[0]], 
           [runner].concat([data, expected, step.out_key])
        );
    
      } else {
        runner.doers[doer[0]].apply([runner].concat([data, expected]));
...
function _callback(runner, data, expected, err) {
  if (err) {
    runner.error(err);

    log(11111111111111111, err);
    ...
    if (checkData(data, expected, runner.parameters)) {
      runner._run(runner); 
