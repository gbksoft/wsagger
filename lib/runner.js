...
function prepareScenarioCall(data, scenarioNum, parameters) {
  ...
  if (s_[scenarioNum] && (typeof s_[scenarioNum] === 'object')) {
    ...
    for (var key of array_(('keysIn' in s_[scenarioNum]) ? s_[scenarioNum].keysIn : data.keysIn)) scenario.parameters[key] = parameters[key];
    scenario.keysOut = ('keysOut' in s_[scenarioNum]) ? s_[scenarioNum].keysOut : data.keysOut;
...
function _run(runner) {
  ...
  while (step = runner.data.shift()) {
    ...
    runner.lastStepKeysOut = step.keysOut;
...
function _callback(runner, step, data, expected, err) {
  ...
  if (step.keysOut != undefined) {
    if (!(step.keysOut instanceof Array)) {
      runner.parameters[step.keysOut] = data; 
      runner.debug('\nstore all data --> ', step.keysOut);
    
    } else if (data && (data instanceof Object)) {
      for (var k of step.keysOut) {
        runner.debug('\nstore ',data[k], ' --> ', k);
