...
fs.writeFileSync('_definitions.json', JSON.stringify(prepared.definitions, null, 3));
...
function convert(prepared, variants, options0) {
  ...
  for (let ps of prepared.scenarios) {
    ...
    let sc = {
      ...
      variants   : {},
      parameters_: [],
      responses  : insertDefinitions(ps.responses, prepared.definitions)
    }; 
    ...
    for (let p of ps.parameters_) { 
      ...
      (p.in === 'body' ? convertBodyParameter : convertParameter)(p, sc.parameters_, p_[p.in], (ps.path === options0.loginPath)); 
    }
...
function convertParameter(p0, parameters_, p__, correctName) { 
  ...
  let name_ = correctName
            ? (p.name === 'email') ? 'user.username' : (p.name === 'password') ? 'user.password' : p.name
            : p.name;
