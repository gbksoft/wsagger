...
let origin   = fs.readFileSync('0.json', 'utf8');
let variants = object_(lib.loadObject('_variants.json'));  
let options0 = { 'url': 'https://gambling-game-api.dev.gbksoft.net/swagger/'};

if (!options0.loginPath) options0.loginPath = '/v1/users/login';
let prepared = prepare(jsonlint.parse(origin)); 
if (!prepared) {
  log('/!prepared');
  return;
}  
...
fs.writeFileSync('_variants.json', JSON.stringify(r.variants, null, 3));
for (let scenario of r.scenarios) fs.writeFileSync(scenario.url, JSON.stringify(scenario, null, 3));
...
function convert(prepared, variants, options0) {
  ...
  addRESTUrl(prepared, variants, options0);
  
  for (let ps of prepared.scenarios) {
    let sn     = '' + ++scenarioNum;
    let scPath = ((ps.path === options0.loginPath) ? loginActionPath : (prefix.substr(sn.length) + sn + (ps.operationId ? '.' + ps.operationId : ''))) 
               + '.wsagger.json';
    ...
    for (let p of ps.parameters) { 
      ...
      (p.in === 'body' ? convertBodyParameter : convertParameter)(p, sc.parameters_, p_[p.in]); 
    }
    ...
    if (ps.consumes && ps.consumes.length) { sc.variants.contentType = ps.consumes; }
    if (ps.produces && ps.produces.length) { sc.variants.Accept      = ps.produces; }

    if (ps.path === options0.loginPath) {
      if      (!d.expected.result)                   { d.expected.result = { 'accessToken': {'token': '{{!token}}'}}; } 
      else if (!d.expected.result.accessToken)       { d.expected.result.accessToken = {'token': '{{!token}}'}; }
      else if (!d.expected.result.accessToken.token) { d.expected.result.accessToken.token = '{{!token}}'; }
...
function convertParameter(p0, parameters_, p__) { 
...
function convertBodyParameter(p0, parameters_, p__) { 
  ...
  if (p0.schema && (typeof p0.schema === 'object') && p0.schema.properties && (typeof p0.schema.properties === 'object')) {
    ...
    for (let p in schema.properties) {
      ...
      convertParameter(p0_, parameters_, p__)
    }

  } else {
     convertParameter(p0, parameters_, p__)
...
function addRESTUrl(prepared, variants, options0) {
  if (options0.url && (typeof options0.url === 'string')) {
    ...
    if (host) {
      ...
      if (0 >= Object.keys(r.variants.REST_).grep((k) => { return string_(r.variants.REST_[k]).toLowerCase() === host; }).length) {
        var key = host;
