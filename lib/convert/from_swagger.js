...
let variants = object_(lib.loadObject('variants.json'));  
let options0 = { 'url': ''};
...
for (let path in r.index) {
  ...
  index += '\n<p><b>' + array_(item.tags).join(', ') + ':</b> <a href="/?url=' + path + '">' + item.name + ' / ' + path + '</a> ' + string_(item.description) + '</p>\n'; 
}
...
for (let scenario of r.scenarios) {
  for (let key in r.variants) {
    if (!(key in scenario.variants)) { scenario.variants[key] = r.variants[key]; }
  }
  let scPath = scenario.scPath; delete scenario.scPath;
  fs.writeFileSync(scPath, JSON.stringify(scenario, null, 3));
}
...
function convert(prepared, variants, options0) {
  ...
  for (let k in prepared.variants) { r.variants[k] = prepared.variants[k]; }
  ...
  for (let ps of prepared.scenarios) {
    ...
    for (let p of ps.parameters) { 
      if (!(p.in in p_)) p_[p.in] = {};
      (p.in === 'body' ? convertBodyParameter : convertParameter)(p, sc.parameters, p_[p.in], options0.loginPaths.includes(ps.path)); 
    }
    ...
    if (ps.consumes) {
      if      (ps.consumes.length >  1) { headers['Content-Type'] = '{{Content-Type}}'; sc.variants['Content-Type'] = ps.consumes; } 
      else if (ps.consumes.length == 1) { headers['Content-Type'] = ps.consumes[0]; }
    }
    
    if (ps.produces) {
      if      (ps.produces.length >  1) { headers['Accept'] = '{{Accept}}'; sc.variants['Accept'] = ps.produces; } 
    ...
    if (options0.loginPaths.includes(ps.path)) {
      ...
      */
      ...
      d.data[1].headers = {}
      if (ps.consumes.length >= 1) { d.data[1].headers['Content-Type'] = ps.consumes[0]; }
      if (ps.produces.length >= 1) { d.data[1].headers['Accept']       = ps.produces[0]; }
  ...
  try {
    for (let key in data) {
      if (key === 'swagger') {
        ...
        if (schemes.length) { prepared.variants.proto = schemes; }
...
function	preparePaths(paths, scenarios) {
  for (let path in paths) {
    ...
    for (let method in paths[path]) {
      ...
      let scenario = {path: path, method: method, parameters: [], responses: {}};
      ...
      for (let key in s0) {
        if (['summary', 'description', 'operationId'].indexOf(key) >= 0) {
          ...
          prepareParameters(array_(s0.context), scenario.parameters);
...
function convertParameter(p0, parameters, p__, correctName) { 
...
function convertBodyParameter(p0, parameters, p__, correctName) { 
  ...
  if (p0.schema && (typeof p0.schema === 'object') && p0.schema.properties && (typeof p0.schema.properties === 'object')) {
    ...
    for (let p in schema.properties) {
      ...
      convertParameter(p0_, parameters, p__, correctName)
    }

  } else {
     convertParameter(p0, parameters, p__, correctName)

   }
}

function prepareParameters(context0, parameters) {

  for (let p0 of context0) {
    let p = {};
    for (let key in p0) {
      if (key === 'in') {
        p[key] = p0[key];
...
function addRESTUrl(r, prepared, options0) {
  if (options0.url && (typeof options0.url === 'string')) {
    ...
    r.variants = object_(r.variants);

    if (host) {
      r.variants.REST_ = object_(r.variants.REST_);
      if (0 >= Object.keys(r.variants.REST_).filter((k) => { return string_(r.variants.REST_[k]).toLowerCase() === host; }).length) {
        var key = host;
        if (key in r.variants.REST_) {
          for (let i = -1; ++i < Object.keys(r.variants.REST_).length;) {
            if (!(((key + i) in r.variants.REST_))) {
              key += i;
              break;

            } else if ((r.variants.REST_[key+i].host == host) && (r.variants.REST_[key+i].path == string_(prepared.basePath))) {
              return;  

            }
          }
        }
        r.variants.REST_[key] = { 
          host: host, 
