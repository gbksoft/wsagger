...
$('body').on('click', '.btn-try', () => {
  ...
  if (scenario.parameters_) {
    for (var p of array_(scenario.parameters_)) {
      // var value = $('#parameters_' + p.name).val ();

      var el = document.getElementById('parameters_' + p.name); 
      if (el) {
         var value = el.value;
  ...
  if (elem.parameters_) {
    ...
    for (var i = 0; i < elem.parameters_.length; i++) {
      ...
      parametersData 
        ...
        + ')</span>:'
        + '<br><input id="parameters_' + itemObj.name + '" value="" placeholder="' + (itemObj.defaultValue ? string_(itemObj.defaultValue) : '') + '" class="formData__name-input form-control">'
        + '</form></div>';
