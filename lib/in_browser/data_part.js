...
function showScenario(elem) {  
  ...
  if (scenario._variants) { variants = scenario._variants; }
  ...
  if (elem.parameters) {
    ...
    for (var i = 0; i < elem.parameters.length; i++) {
      ...
      parametersData 
        ...
        + ')</span>:'
        + '<br><input id="parameters' + itemObj.name + '" value="" placeholder="' + (itemObj.defaultValue ? string_(itemObj.defaultValue) : '') + '" class="formData__name-input form-control">'
        + '</form></div>';
  ...
  for (var v in elem) {
    ...
    if (['wsagger', 'info', 'scPath', '_variants', 'responses', 'parameters'].includes(v)) {
      continue;
