...
$('body').on('click', '.btn-try', () => {
  ...
  if (scenario.parameters) {
    for (var p of array_(scenario.parameters)) {
      // var value = $('#parameters' + p.name).val ();

      var el = document.getElementById('parameters' + p.name); 
      if (el) {
         var value = el.value;
...
function showVariants(elem) { 
  ...
  $('#Variants').html(t ? '\xA0<table width=100%>' + t + '</table>\n' : noVariantsText);
}

function showExpectations(elem) { 
  expectations = object_(elem);
  var t = '';
...
function showScenario(elem) {  
  ...
  if (scenario.variants) { variants = scenario.variants; }
  ...
  if (elem.parameters) {
    ...
    for (var i = 0; i < elem.parameters.length; i++) {
      var itemObj = elem.parameters[i];
      ...
      parametersData 
        ...
        + ')</span>:'
        + '<br><input id="parameters' + itemObj.name + '" value="" placeholder="' + string_(('default' in itemObj) ? itemObj.default : itemObj.defaultValue) + '" class="formData__name-input form-control">'
        + '</form></div>';
  ...
  for (var v in elem) {
    ...
    if (['wsagger', 'info', 'scPath', 'variants', 'responses', 'parameters'].includes(v)) {
      continue;
    
    } else if (['data', 'functions'].includes(v)) {
      divOrPre = 'pre';
  ...
  $('#jsonloader').find('.feedback').removeClass('error').addClass('success').html( "JSON was loaded successfully" ).delay(1000).fadeOut();
